...
export default function AiBenchCard({ quarter, vendor, data }) {
  // Total headcount, manual hours saved, and adoption % are real,
  // filtered numbers from revenueData.js. The role-by-role breakdown
  // (AI Engineer, Data Engineer, etc.) has no quarter/vendor-tagged
  // source yet, so those individual counts stay static until that
  // data exists.
  ...
  const liveData = data ?? {
    ...SAMPLE_DATA,
    adoption: {
      ...SAMPLE_DATA.adoption,
      manualWorkSaved: summary.totalManualHoursSaved,
      adoptionPercentage: summary.avgAiAdoption,
    },
    bench: {
      ...SAMPLE_DATA.bench,
      totalHeadcount: summary.headcountTotal,
    },
  };

  const adoption = liveData?.adoption ?? {};
  const bench = liveData?.bench ?? {};
